...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    selected_paragraphs = list(filter(select, paragraphs))
...
def accuracy(typed, source):
    ...
    for i in range(len(typed_words)):
        if i < len(source_words) and typed_words[i] == source_words[i]:
            ...
            error_count += 1
    return (len(typed_words) - error_count) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    match_list = [word for word in word_list if diff_function(typed_word, word,limit) <= limit]
    
    if match_list == []:
        return typed_word
    else:
        return typed_word if typed_word in word_list else min(match_list, key=diff)
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 and len(source) == 0:
        ...
        if (len(typed) == 0 or len(source) == 0) or typed[0] != source[0]:
...
def time_per_word(words, timestamps_per_player):
    ...
    for j in range(len(timestamps_per_player)):
        time = [timestamps_per_player[j][i+1] - timestamps_per_player[j][i] for i in range(len(timestamps_per_player[j])-1)]
        result.append(time)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_list = [[] for i in player_indices]
    for word_index in word_indices:
        word = get_word(match,word_index)
        time_list = [time(match, i, word_index) for i in player_indices]
        index_fastest_player = time_list.index(min(time_list))
        fastest_list[index_fastest_player].append(word)
